module Bitgo
  module V1
    ...
    class Api
      ...
      def login(email: self.email, password: self.password, otp: self.otp)
        login_params = {
            email: email,
            password: password,
            otp: otp
      ...
      def unlock(otp: self.otp, duration_seconds: self.duration_seconds)
        unlock_params = {
            otp: otp,
      ...
      def add_keychain(xpub: self.xpub, encrypted_xprv: self.encrypted_xprv)
      ...
      def list_labels_for_wallet(wallet_id: self.wallet_id)
        call :get, '/labels/' + wallet_id
      end

      def set_label(wallet_id: self.wallet_id, address: self.address, label: self.label)
        call :put, '/labels/' + wallet_id + '/' + address, {label: label}
      end

      def delete_label(wallet_id: self.wallet_id, address: self.address)
      ...
      # }
      def simple_create_wallet(passphrase: self.passphrase, label: self.label)
      ...
      def add_wallet(label: self.label, m: self.m, n: self.n, keychains: self.keychains, enterprise: nil)
      ...
      def create_address(wallet_id: self.wallet_id, chain: self.chain)
        call :post, '/wallet/' + wallet_id + '/address/' + chain
      end

      def send_coins_to_address(wallet_id: self.wallet_id, address: self.address, amount: self.amount, wallet_passphrase: self.wallet_passphrase, min_confirms: self.min_confirms, enforce_min_confirms: self.enforce_min_confirms)
        payload = {
            address: address,
            amount: amount,
        ...
        #add optional params
        payload[:minConfirms] = min_confirms if min_confirms
        payload[:enforceMinConfirmsForChange] = enforce_min_confirms if enforce_min_confirms
      ...
      def add_webhook(wallet_id: self.wallet_id, type: self.type, url: self.url, confirmations: self.confirmations)
        add_webhook_params = {
            type: type,
            url: url,
      ...
      def remove_webhook(wallet_id: self.wallet_id, type: self.type, url: self.url)
      ...
      def list_webhooks(wallet_id: self.wallet_id)
      ...
      def encrypt(input: self.input, password: self.password)
        call :post, '/encrypt', {input: input, password: password}
      end

      def decrypt(input: self.input, password: self.password)
      ...
      def verify_address(address: self.address)
